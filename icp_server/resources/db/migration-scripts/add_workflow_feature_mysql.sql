...
SET @col_exists = (SELECT COUNT(*) FROM information_schema.COLUMNS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'runtimes' AND COLUMN_NAME = 'callback_url');
SET @ddl = IF(@col_exists = 0, 'ALTER TABLE runtimes ADD callback_url VARCHAR(500) NULL', 'SELECT 1');
PREPARE stmt FROM @ddl;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- 2. Allow the 'Workflow-Management' permission domain (no-op when already widened)
