...
CREATE INDEX idx_sso_group_mapping_org ON sso_group_mappings (org_uuid);
...
CREATE INDEX idx_fed_group_user_user ON federated_group_user_mapping (user_uuid);

CREATE INDEX idx_fed_group_user_group ON federated_group_user_mapping (group_id);

CREATE INDEX idx_fed_group_user_issuer_claim ON federated_group_user_mapping (issuer, claim_name, claim_value);
...
CREATE INDEX idx_bi_automation_execution_history_runtime_id ON bi_automation_execution_history (runtime_id);

CREATE INDEX idx_bi_automation_execution_history_package_name ON bi_automation_execution_history (package_name);

CREATE INDEX idx_bi_automation_execution_history_execution_timestamp ON bi_automation_execution_history (execution_timestamp);
...
CREATE TABLE mi_data_service_artifacts (
    ...
    composite_app VARCHAR(200),
    error_message CLOB, -- Error message when state is Faulty (data service failed to deploy)
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE INDEX idx_mi_composite_app_artifacts_runtime_id ON mi_composite_app_artifacts (runtime_id);

CREATE INDEX idx_mi_composite_app_artifacts_app_name ON mi_composite_app_artifacts (app_name);

CREATE INDEX idx_mi_composite_app_artifacts_state ON mi_composite_app_artifacts (state);
